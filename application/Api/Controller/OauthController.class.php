...
class OauthController extends HomebaseController {
	...
	public function callback($type = null, $code = null){
		...
		if(is_array($token)){
			...
			//$res=M('OauthUser')->where(array('name'=>$user_info['name']))->find();
			if($_SESSION['user']&&$_SESSION['user']['is_bang']==0){
	...
	private function _bang_handle($user_info, $type, $token){
		...
		if($find_oauth_user){
			...
			if($find_oauth_user['uid']==$current_uid){
				$this->error("您之前已经绑定过此账号！",U('user/account/index'));exit;
			}else{
				$this->error("该帐号已被本站其他账号绑定！",U('user/account/index'));exit;
			}
		...
		if($need_bang){
			...
			if($current_uid){
				...
				if($new_oauth_user_id){
					$this->success("绑定成功！",U('user/account/index'));
				}else{
					$users_model->where(array("id"=>$new_user_id))->delete();
					$this->error("绑定失败！",U('user/account/index'));
				}
			}else{
				$this->error("绑定失败！",U('user/account/index'));
			}
	...
	private function _login_handle($user_info, $type, $token){
		...
		$need_register=true;
		if($find_oauth_user){
			$oauth_user_model->where(array("from"=>$type,"openid"=>$token['openid']))->save(array('status'=>1));
