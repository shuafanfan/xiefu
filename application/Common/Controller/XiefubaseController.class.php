...
class XiefubaseController extends AppframeController {
    ...
    protected function page($total_size = 1, $page_size = 0, $current_page = 1, $listRows = 6, $pageParam = '', $pageLink = '', $static = FALSE) {
        ...
        $Page->SetPager('Xiefu', '{first}{prev}&nbsp;{liststart}{list}{listend}&nbsp;{next}{last}', array("listlong" => "9", "first" => "首页", "last" => "尾页", "prev" => "上一页", "next" => "下一页", "list" => "*", "disabledclass" => ""));
        return $Page;
    }
	 ...
	 */
	public $formatTree; //用于树型数组完成递归格式的全局变量
...
//	}
	...
	//无限递归获取子数据ID集合
	private $childIds;
	private function _getChildIds($model,$con,$pid = '0', $pk_str='id' , $pid_str ='pid')
	{
		$where  =$pid_str."=".$pid;
		$where .=$con;
		$childItem_arr = M($model)->where($where)->select();
		if($childItem_arr)
		{
			foreach($childItem_arr as $childItem)
			{
				$this->childIds[] = $childItem;
				$this->_getChildIds($model,$con,$childItem[$pk_str],$pk_str,$pid_str);
			}
		}
	}
	public function getChildIds($model,$con,$pid = '0', $pk_str='id' , $pid_str ='pid')
	{
		$this->childIds = array();
		$this->_getChildIds($model,$con,$pid,$pk_str,$pid_str);
		return $this->childIds;
	}
}