...
class AdminProductController extends AdminbaseController {
		...
		if (IS_POST) {
			...
			if ($result) {
				...
				foreach ($_POST['term'] as $mterm_id){
					$this->term_relationships_model->add(array("term_id"=>intval($mterm_id),"product_id"=>$result));
				}
	...
	public function edit(){
		...
		$term_relationship = M('TermRelationships')->where(array("product_id"=>$id,"status"=>1))->getField("term_id",true);
	...
	public function edit_post(){
		...
		if (IS_POST) {
			...
			$this->term_relationships_model->where(array("product_id"=>$post_id,"term_id"=>array("not in",implode(",", $_POST['term']))))->delete();
			foreach ($_POST['term'] as $mterm_id){
				$find_term_relationship=$this->term_relationships_model->where(array("product_id"=>$post_id,"term_id"=>$mterm_id))->count();
				if(empty($find_term_relationship)){
					$this->term_relationships_model->add(array("term_id"=>intval($mterm_id),"product_id"=>$post_id));
				}else{
					$this->term_relationships_model->where(array("product_id"=>$post_id,"term_id"=>$mterm_id))->save(array("status"=>1));
				}
			}
	...
	private  function _lists($status=1){
		...
		->alias("a")
		->join(C ( 'DB_PREFIX' )."product b ON a.product_id = b.id")
		->where($where)
		->count();
	...
	private function _getTree(){
		...
		$result = $this->terms_model->where('taxonomy="product"')->order(array("listorder"=>"asc"))->select();
	...
	function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			$tids=join(",",$_POST['ids']);
			$objectids=$this->term_relationships_model->field("product_id")->where("tid in ($tids)")->select();
			$ids=array();
